...
const fetchTasks = async (id_task) => {
    const response = await fetch(`http://localhost:3333/tasks${id_task}`)
...
const displayTaskDetails = async () => {
    ...
    // Atualiza os campos do formulário com os detalhes da tarefa
    document.getElementById('taskId').value = taskDetails.id_task;
    document.getElementById('taskTitle').value = taskDetails.title;
    document.getElementById('taskDetails').value = taskDetails.details || ''; // Se 'details' for null, define como uma string vazia


}

// Chama a função para exibir os detalhes da tarefa quando a página carrega
